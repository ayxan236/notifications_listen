...
class NotificationsHandlerService: MethodChannel.MethodCallHandler, NotificationListenerService() {
    ...
    private fun sendEvent(evt: NotificationEvent) {
        ...
        Log.d(TAG, "send notification event: ${evt.data["title"]}")
        val timestamp = evt.data["timestamp"] as? Long ?: 0L // Получаем значение из evt.data["timestamp"]
        val roundedTimestamp = (timestamp / 1000).toDouble().roundToInt()
        val notificationMap = mapOf(

        "message" to evt.data["text"],
        "packageName" to evt.data["package_name"],
        "title" to evt.data["title"],
        "timeStamp" to roundedTimestamp,
        )
    ...
    private fun sendHttpRequest(json: String,packageName:String?) {
        ...
        client.newCall(request).enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                Log.d(TAG, "send notification response: ${responseData}")
            }
                // responseData содержит ответ сервера
            }
        })
    }
